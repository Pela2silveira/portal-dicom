...
* **Ingreso opcional por usuario y contraseña (cuenta Andes):** además del código por mail, el paciente puede ingresar con las mismas credenciales de la cuenta Andes/Mi Salud. Es un método aditivo controlado por `patient.password_login_enabled` (default `false`), independiente de `auth_mode`. El backend delega la validación server-to-server en la API mobile de Andes (`POST {ANDES_MOBILE_API_BASE_URL}/modules/mobileApp/login`), obtiene el `documento`, resuelve el paciente y emite la sesión habitual; el JWT de Andes no llega al browser. Cuando el flag está activo, `ANDES_MOBILE_API_BASE_URL` es obligatorio.
