...
* **TO-DO de OHIF lazy loading:** queda pendiente investigar si conviene ajustar el modo de carga de metadata del visor OHIF sobre Orthanc local, incluyendo la evaluación de `dicom-json` con `Preload=false` versus el datasource actual `dicom-web`, midiendo impacto en tiempo de primera apertura, aperturas subsiguientes y costo de precálculo en Orthanc.
