...
* **Mapeo DNI→Codigo vía HIS legacy MSSQL (v0.8.0):** para nodos con `patient_id_source: "legacy_his"` (hoy solo `hpnsyn`), la búsqueda por DNI debe mapear el DNI al código interno de paciente del HIS (`Historias_Clinicas.Codigo`) consultando una base Microsoft SQL Server legacy (`Hospital.dbo.Historias_Clinicas`, filtro `HC_Documento`) y enviar ese `Codigo` como `PatientID` DICOM. La conexión se configura en el bloque `legacy_his` de `config.json` (host, puerto, base, usuario y `password_env`); la contraseña se lee de la variable de entorno, nunca del JSON. El mapeo se cachea en `patient_identifiers` (`legacy_his_code`) y se resuelve en vivo ante un miss. Si la base legacy está caída o el DNI no existe, el nodo no devuelve resultados en lugar de fallar la búsqueda global.
...
* **Modo de auth paciente por config:** el backend debe permitir seleccionar el/los método(s) de acceso paciente mediante `patient.auth_mode` en `config.json`, con valores `mail` (código por mail), `api` (usuario/contraseña Andes), `both` (ambos), más `fake_auth` y `master_key`. `auth_mode` es la única fuente de verdad: no existe un flag separado para el ingreso por contraseña.
* **Método final esperado para paciente:** `patient.auth_mode = "mail"` es el camino final de producción; `both` habilita adicionalmente el ingreso por usuario/contraseña.
* **Ingreso por usuario y contraseña (cuenta Andes):** con `auth_mode` en `api` o `both`, el paciente puede ingresar con las credenciales de la cuenta Andes/Mi Salud. El backend delega la validación server-to-server en la API de Andes (`POST {HIS_BASE_URL}/modules/mobileApp/login`, misma base que el proveedor REST de prestaciones), obtiene el `documento`, resuelve el paciente y emite la sesión habitual; el JWT de Andes no llega al browser.
