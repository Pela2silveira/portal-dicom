...
* En la carga inicial del paciente, ese endpoint debe ejecutar `QIDO-RS /studies?PatientID=<dni>` contra el único nodo PACS configurado usando exactamente los filtros recibidos del formulario si existen; luego debe persistir los estudios sincronizados y reutilizar esa cache local para los filtros posteriores del paciente.
