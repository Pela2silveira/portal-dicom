...
- [Ready] **First patient sync uses form filters**: if patient cache is empty, the initial remote sync uses the same date/modality filters received from the patient form rather than a separate backend-only horizon.
