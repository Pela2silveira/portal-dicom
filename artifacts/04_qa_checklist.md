...
- [Ready] **Patient study share links**: the patient surface can issue short-lived study-scoped share links for local studies; `/share?t=...` consumes one use, sets the share token cookie, and redirects into Stone/OHIF while Orthanc/DICOMweb authorization is enforced through the resource-token validation path.
