...
- [Ready] **Orthanc internal-vs-viewer auth split**: browser/viewer access still requires a valid viewer grant cookie, while backend internal calls to Orthanc use `X-Orthanc-Internal-Token` and the authorization plugin `user profile` + `Permissions` path for admin/internal routes (`/tools/find`, `/modalities/*`, `/jobs/*`, local archives).
