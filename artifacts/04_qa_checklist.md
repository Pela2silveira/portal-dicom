...
- [Ready] **Professional local-cache manual broad search**: the initial physician load may stay bounded by `professional.initial_cache_period`, but an explicit manual `Buscar` on `source=local_cache` without extra filters must return the full local Orthanc cache.
...
- [Ready] **Patient study share links**: the patient surface can issue short-lived study-scoped share links for local studies; `/share?t=...` consumes one use, sets the share token cookie, and redirects into Stone/OHIF while Orthanc/DICOMweb authorization is enforced through the resource-token validation path. The generated link lifetime follows the share-link policy itself and is not clipped to the creator's remaining session time. The patient card now exposes a single `Compartir` CTA that opens a local share panel with QR, WhatsApp, generic share, and copy-link actions built from the same magic link.
