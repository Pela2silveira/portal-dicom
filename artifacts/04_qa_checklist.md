...
- [Ready] **Retrieve percentage on hover**: while a retrieve is `queued`/`running`, the action button exposes the current percentage through its hover `title`, including `0%` when that is the only value reported by Orthanc.
