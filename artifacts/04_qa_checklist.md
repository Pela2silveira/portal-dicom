...
- [Ready] **Patient entry keyboard flow**: the public patient form starts with focus on `Documento`; `Tab` from that field moves to `Enviar código`; a successful send-code action moves focus to `Código por mail`; and `Tab`/`Enter` from the code input moves focus to `Continuar`.
- [Ready] **Professional entry keyboard flow**: when the professional role is active, the public form starts with focus on `DNI`; `Tab`/`Enter` from `DNI` moves to `Contraseña`; and `Tab`/`Enter` from `Contraseña` moves focus to `Continuar`.
