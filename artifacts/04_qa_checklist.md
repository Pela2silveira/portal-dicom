...
- [Ready] **Orthanc internal-vs-viewer auth split**: backend internal calls to Orthanc (`/dicom-web` local cache checks, `/tools/find`, `/modalities/*`, `/jobs/*`, local archives) use `X-Orthanc-Internal-Token`, while browser/viewer access still requires a valid viewer grant cookie.
