...
- [Ready] **Landing role-selector keyboard flow**: the public landing starts with focus on the active role button; natural `Tab` moves between `Paciente` and `Profesional`; and `Enter` on either role jumps to that role's user-identification input.
...
- [Ready] **Startup grace for remote PACS health**: optional remote PACS checks stay `unknown` during a short backend startup grace period, avoiding immediate degraded state from cold-start races.
