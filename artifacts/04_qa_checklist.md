...
- [Ready] **Patient study share links**: the patient surface can issue short-lived study-scoped share links for local studies; `/share?t=...` consumes one use, sets the share token cookie, and redirects into Stone/OHIF while Orthanc/DICOMweb authorization is enforced through the resource-token validation path. The generated link lifetime follows the share-link policy itself and is not clipped to the creator's remaining session time. The patient card exposes `WhatsApp` as the primary share CTA and `Compartir` as a secondary generic fallback.
