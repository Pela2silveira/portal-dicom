...
- [Ready] **ANDES enrichment on result cards**: patient and professional cards expose `Prestación en ANDES` and `Profesional en ANDES` when Mongo `prestaciones` contains a match by `metadata.pacs-uid`; patient uses the persisted patient Mongo id and professional uses the node `andes_organization_id` plus study date.
