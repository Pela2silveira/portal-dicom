...
- [Ready] **Visible retrieve notice in workspace**: while a patient or professional retrieve is queued/running, the workspace shows a prominent notice above results in addition to the row/button state, so progress remains visible even when Orthanc reports `0%` for most of the job.
