...
- Además del código por mail, el paciente puede opcionalmente ingresar con **usuario y contraseña de la cuenta Andes (Mi Salud)**. Es un método aditivo controlado por el flag `patient.password_login_enabled` (independiente de `auth_mode`): con `false` (default) el flujo visible no cambia. El endpoint `POST /api/patient/password-login` recibe `{ email, password }`, delega server-to-server en `POST {HIS_BASE_URL}/modules/mobileApp/login` (igual que el profesional delega en LDAP), extrae el `documento` de la respuesta/JWT, resuelve el paciente con el mismo flujo de identidad y emite la sesión de paciente habitual. El JWT de Andes nunca llega al browser ni se usan sus permisos (shiro-trie); solo la identidad. La firma del JWT no se verifica: es una llamada server-to-server sobre TLS y el `documento` solo se usa para resolver un paciente existente. La base es `HIS_BASE_URL` (compartida con el proveedor REST de prestaciones; default `https://app.andes.gob.ar/api`), sin variable de entorno dedicada.
