...
- En la UI de paciente, compartir estudio local debe exponerse con `WhatsApp` como CTA principal que abre `wa.me`/WhatsApp Web con el enlace mágico ya armado, `Compartir` como acción secundaria genérica que usa share nativo del navegador cuando existe y, si no, cae a copiar el enlace o mostrarlo, y `QR` como acción adicional que presenta un código QR generado localmente a partir del mismo enlace mágico.
