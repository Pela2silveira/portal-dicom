...
- El modo de auth paciente debe poder alternarse por config (`patient.auth_mode`) para seleccionar el/los método(s): código por correo (`mail`), usuario/contraseña Andes (`api`), ambos (`both`), demo (`fake_auth`) y llave maestra (`master_key`), sin cambiar endpoints. `auth_mode` es la única fuente de verdad de la selección de método.
...
- Además del código por mail, el paciente puede ingresar con **usuario y contraseña de la cuenta Andes (Mi Salud)** cuando `patient.auth_mode` es `api` o `both`. El endpoint `POST /api/patient/password-login` recibe `{ email, password }`, delega server-to-server en `POST {HIS_BASE_URL}/modules/mobileApp/login` (igual que el profesional delega en LDAP), extrae el `documento` de la respuesta/JWT, resuelve el paciente con el mismo flujo de identidad y emite la sesión de paciente habitual. El JWT de Andes nunca llega al browser ni se usan sus permisos (shiro-trie); solo la identidad. La firma del JWT no se verifica: es una llamada server-to-server sobre TLS y el `documento` solo se usa para resolver un paciente existente. La base es `HIS_BASE_URL` (compartida con el proveedor REST de prestaciones; default `https://app.andes.gob.ar/api`), sin variable de entorno dedicada.
...
- La resolución del estado operacional por estudio no debe emitir un `POST /tools/find` redundante a Orthanc cuando el llamador ya sabe que el estudio está presente localmente (por ejemplo, la búsqueda en `local_cache`, donde cada resultado provino de ese mismo `/tools/find`). El listado local resuelve el estado sin ese lookup extra para no convertir una búsqueda en 1+N consultas y agravar la contención del índice de Orthanc.
- El C-ECHO DIMSE puede usar un timeout levemente mayor para reducir falsos negativos sin introducir gracia de arranque ni warm-up automático de PACS remotos.
...
- **UI Portal Pública (MVP)**: landing estática servida por Nginx para:
  ...
  - Permitir el acceso sólo si el documento existe y cumple `habilitado == true` más matrícula activa; los DNI/username incluidos en `professional.license_exceptions` saltean ambos checks.
...
10. Si el backend reinicia con búsquedas paciente todavía en `queued` o `running`, debe reconciliarlas a `failed` al arrancar para no rehidratar un `Buscando...` huérfano desde Postgres.
11. El worker actualiza `patient_study_access`, marca como `available_local` sólo los estudios que Orthanc ya confirma locales y deja el resto como `pending_retrieve`.
12. El flujo debe dejar trazas estructuradas de observabilidad para:
...
12. Cuando el profesional aplica filtros con `source=local_cache`, `GET /api/physician/results` debe filtrar sobre Orthanc local; si el profesional ejecuta manualmente `Buscar` sobre `local_cache` sin filtros adicionales, el portal debe devolver toda la cache local en lugar de reimponer `professional.initial_cache_period`. Con `source=<node_id>`, debe ejecutar QIDO-RS únicamente contra ese nodo remoto y persistir el resultado como búsqueda reciente.
    - La consulta al caché local del profesional debe usar `POST /tools/find` (Study level, `Expand`, `RequestedTags` para `ModalitiesInStudy`/`NumberOfStudyRelatedInstances`) y **no** QIDO interno `GET /dicom-web/studies?PatientID=...`: bajo el plugin de autorización (`CheckedLevel: studies`) una QIDO filtrada por `PatientID` de un paciente sin estudios cacheados se rechaza con `403 Unknown resource` (ver también punto de autorización sobre `/tools/find`).
...
- Los servicios principales del compose (`nginx`, `backend`, `postgres`, `orthanc`, `ohif`) deben declarar `restart: unless-stopped` para que Docker los reinicie automáticamente tras un reboot de la VM sin requerir un `docker compose up` manual.
...
- `pacs_nodes[].patient_id_source` (opcional, default `"dni"`): regla de mapeo que decide qué identificador del paciente viaja como `PatientID` DICOM cuando la búsqueda profesional es por DNI. Valores soportados: `"dni"`, `"mongo_id"`, `"legacy_his"` (alias `"mssql_codigo"`/`"codigo"`); otros valores quedan reservados para campos de otros HIS providers (aún no implementados).
- `legacy_his` (opcional): conexión al HIS legacy Microsoft SQL Server usado para mapear DNI→`Codigo` en nodos con `patient_id_source: "legacy_his"`. Campos: `enabled`, `host`, `port` (default 1433), `database` (default `Hospital`), `user`, `password_env` (default `LEGACY_HIS_PASSWORD`, la contraseña se lee de esa env var), `encrypt` (default `disable`), `connect_timeout_ms`, `query_timeout_ms`. Query: `SELECT TOP 1 [Codigo] FROM [<db>].[dbo].[Historias_Clinicas] WHERE HC_Documento = @dni`. Driver `github.com/microsoft/go-mssqldb` fijado en `v1.7.2` (última compatible con Go 1.22). El mapeo se cachea en `patient_identifiers` (`source_system=legacy_his`, `identifier_type=legacy_his_code`, best-effort sobre pacientes ya existentes). Arranque resiliente: si falla construir/pingear, se registra startup issue y el cliente queda nil (no bloquea boot).
...
- `GET /api/physician/results`:
  ...
  - DNI vs ID (v0.7.0): acepta `patient_id` (ID DICOM directo, se envía tal cual como `PatientID`) y `document_number` (DNI). `patient_id` tiene precedencia. Para `document_number`, el backend resuelve identificadores (cache `patient_identifiers` primero, refresco Mongo/HIS del `mongo_id` sólo si hace falta) y por cada nodo aplica su `patient_id_source` (`dni`|`mongo_id`|`legacy_his`|campo futuro) para elegir el `PatientID` a enviar; si no puede mapear, ese nodo no devuelve resultados (log `physician_search_dni_unmapped`). En `local_cache` consulta por cada identificador mapeado distinto y deduplica por `StudyInstanceUID`.
  - HIS legacy MSSQL (v0.8.0): para `patient_id_source: "legacy_his"` (hoy `hpnsyn`), la resolución del DNI incluye un lookup DNI→`Codigo` contra SQL Server (`resolveLegacyHISCode`): lee la cache `patient_identifiers` (`legacy_his_code`) y ante miss consulta el MSSQL en vivo, persistiendo best-effort el mapeo. `resolvePatientSearchIdentifiers` recibe ahora `patientIdentifierNeeds{Mongo, LegacyHIS}` para disparar cada lookup externo sólo cuando algún nodo configurado lo requiere. Logs: `legacy_his_lookup_failed`, `legacy_his_cache_failed`, `legacy_his_persist_failed`.
...
- El portal puede además emitir `resource tokens` de estudio puntual para compartir estudios locales del paciente mediante enlaces mágicos (`/share?t=...`); estos tokens se validan por `WebServiceTokenValidationUrl`, pueden decodificarse por `WebServiceTokenDecoderUrl`, y deben quedar acotados a un único `StudyInstanceUID`, expiración corta y cantidad máxima de usos. La expiración del enlace debe depender de la política del share link y no del vencimiento de la sesión paciente que originó la operación.
- En la UI de paciente, compartir estudio local debe exponerse con un único CTA `Compartir`; ese CTA debe abrir un panel local con el código QR del enlace mágico y acciones secundarias para `WhatsApp`, share genérico del navegador y copia del enlace.
