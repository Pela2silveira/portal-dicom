...
- The portal favicon must be served by the portal/Nginx surface itself as a real local `/favicon.ico` file; browser-visible favicon selection should prefer that `.ico` file over optional SVG assets.
...
- In mobile widths, the share QR dialog header should stack vertically so the `Cerrar` action does not compress the title or body copy.
...
- when the physician enters the workspace without filters, the first visible result set should be the local cached studies for the current calendar week
