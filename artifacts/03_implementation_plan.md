...
- **[Done] Logging Opción A (2026-05-31)**: streaming del stdout de `docker compose` a archivos planos en `app/logs/` del remoto (`portal.log` firehose + `audit.log`), vía units systemd + `logrotate` (retención ~3 meses). Provisión y reenganche en deploy/update desde `Makefile.deploy.local` (`logging-install`/`logging-restart`/`logging-status`). Ver `decisions.md` (2026-05-31).
- **[Done] RBAC + auditoría + métricas (acción transversal) (2026-05-31)**: catálogo de acciones en código (`rbac.go`, `actionCatalog`) + decorador `App.action(id, next)` que centraliza same-origin (mutantes), resolución de actor, RBAC, auditoría y métrica. RBAC simple por `config.json` (`rbac.roles`/`physician_sharers`, sin IdP). Auditoría a stdout con `"channel":"audit"` (esquema who/when/what/subject/result + dims), ya ruteada a `audit.log`. Métricas = Opción B: tabla append-only `usage_events` (migración `010`) detrás de la interfaz `UsageRecorder` (`PostgresUsageRecorder`). En producción, enganchado en logins (paciente/profesional, con identificador+resultado+IP), `viewer_access.grant`, `study.retrieve`, `study.download`, `share_link.create` y `share_link.consume`; `share_link.revoke` declarada (sin endpoint aún). PHI: se difiere la minimización (hoy se loguea todo). Tests en `rbac_test.go` (verde con `make test-backend` + `go vet`). Ver `decisions.md` → "RBAC + auditoría + métricas".
- **[TBD] Pendiente de acción**: enganchar `share_link.revoke` cuando exista endpoint de revocación, y (cuando se trabaje PHI) materializar la política `PHIFields` por acción para redactar campos sensibles en auditoría.
