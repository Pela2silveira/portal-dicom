...
- The Orthanc authorization plugin must also expose `WebServiceUserProfileUrl` for the backend internal token because admin/internal routes such as `/tools/find`, `/modalities/*`, `/jobs/*`, and `/studies/{id}/archive` are authorized through plugin permissions rather than only through study resource-token parsing.
