...
- Patient study sharing is now supported as a study-scoped magic link: the portal issues short-lived resource tokens for a single local `StudyInstanceUID`, exposes a share landing `/share?t=...`, and relies on the Orthanc authorization plugin resource-token validation path to authorize viewer/DICOMweb access without requiring the recipient to hold an active portal session.
