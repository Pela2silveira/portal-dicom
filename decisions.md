...
- In the patient study card UX, WhatsApp is the primary share action and generic share is secondary: the first CTA should prefer the explicit `WhatsApp` deep link, while `Compartir` remains as a fallback to native browser sharing / clipboard flows. A dedicated `QR` action may reuse the same magic link, but the QR itself must be generated locally by the portal and never through a third-party service.
